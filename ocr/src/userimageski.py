...
class UserData():
    ...
    def __init__(self, image_path=None, image=None, postagestam_shape=(64,64)):
        """
        reads the image provided by the user as grey scale and preprocesses it.
        """
        self.postagestam_shape = postagestam_shape
        if image_path is not None:
            self.image = imread(image_path, as_grey=True)
        elif image is not None:
            self.image = image.copy()
        else:
            raise ValueError('Must pass argument')
        ...
        for region in regionprops(label_image):
            if region.area > 10:
                ...
                if roi.shape[0]*roi.shape[1] == 0:
                    ...
                    if i==0:
                        samples = resize(roi, self.postagestam_shape)
                        coordinates.append(region.bbox)
                        i+=1
                    elif i==1:
                        roismall = resize(roi, self.postagestam_shape)
