...
class SWTScrubber(object):
    ...
    def scrub(self):
        ...
        """
        canny, sobelx, sobely, theta = self._create_derivative(self.filepath)
    ...
    @classmethod
    def _create_derivative(self):
        img = sk.io.imread(self.filepath, as_grey=True)
        edges = sk.feature.canny(img, sigma=1)
        # Create gradient map using Sobel
        sobelx64f = sk.filters.sobel_h(img)
        sobely64f = sk.filters.sobel_v(img)
    ...
    def get_mag(self):
        canny, sobelx, sobely, theta = self._create_derivative(self.filepath)
        mag_g = np.sqrt(sobelx * sobely + sobely * sobelx)
        return mag_g
        ...
        for y, x in zip(*np.where(edges)):
            ...
            while True:
                ...
                if cur_x != prev_x or cur_y != prev_y:
                    ...
                    try:
                        if edges[cur_y, cur_x]:
                            # found edge,
                            ray.append((cur_x, cur_y))
                            theta_point = theta[y, x]
                            alpha = theta[cur_y, cur_x]
                            if math.acos(round(grad_x * -grad_x_g[cur_y, cur_x] + grad_y * -grad_y_g[cur_y, cur_x],2)) < np.pi / 2.0:
    ...
    def _find_letters(self, swt, shapes):
        ...
        for label, layer in shapes.iteritems():
            ...
            if diagnostics:
                ...
                sk.io.imsave('layer' + str(label) + '.jpg', layer * 255)
    ...
    def _find_words(self, swts, heights, widths, topleft_pts, images):
        ...
        for chain in [c for c in chains if len(c) > 3]:
            for idx in chain:
                ...
                # sk.io.imsave('keeper'+ str(idx) +'.jpg', images[idx] * 255)
                # final += images[idx]
